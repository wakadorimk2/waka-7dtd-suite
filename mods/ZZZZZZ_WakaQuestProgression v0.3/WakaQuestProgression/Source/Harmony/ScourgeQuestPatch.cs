    ...
    public static class ScourgeQuestPatch
    {
        ...
        public static void Prefix(object __instance, object[] __args, out State __state)
        {
            ...
            try
            {
                ...
                {
                    Log.Out($"[WakaQuestProgression] Tier {tier} variant '{newId}' not registered, keeping '{orig}'");
                    return;
                }

                if (!ScourgeBridge.WriteQuestId(__instance, newId)) return;
                __state = new State { OriginalQuestId = orig, Restore = true };
                Log.Out($"[WakaQuestProgression] Redirected '{orig}' -> '{newId}' (POI tier {tier})");
            }
            catch (Exception e)
            {
                Log.Warning($"[WakaQuestProgression] Prefix swap failed: {e.Message}");
            }
        }
        ...
        public static void Postfix(object __instance, State __state)
        {
            ...
            {
                Log.Warning($"[WakaQuestProgression] Postfix restore failed: {e.Message}");
            }
        }
