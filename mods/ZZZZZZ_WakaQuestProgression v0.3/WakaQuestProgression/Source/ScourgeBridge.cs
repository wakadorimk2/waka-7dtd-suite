    ...
    public static class ScourgeBridge
    {
        ...
        public static void Initialize()
        {
            try
            {
                ...
                {
                    Log.Warning($"[WakaQuestProgression] {TargetTypeName} not found; tier redirect disabled");
                    return;
                }
                ...
                {
                    Log.Warning($"[WakaQuestProgression] {ItemActionType.FullName}.Properties field not found; redirect disabled");
                    return;
                }

                DynamicPropertiesType = PropertiesField.FieldType;
                Log.Out($"[WakaQuestProgression] Properties field resolved: type={DynamicPropertiesType.FullName}");
                ...
                if (DpGetStringMethod != null)
                    Log.Out($"[WakaQuestProgression] Found {DynamicPropertiesType.FullName}.GetString(string)->string");
                ...
                if (DpSetStringMethod != null)
                    Log.Out($"[WakaQuestProgression] Found writer: {DynamicPropertiesType.FullName}.{DpSetStringMethod.Name}(string,string)->void");
                ...
                if (ValuesField != null)
                {
                    ...
                    Log.Out($"[WakaQuestProgression] Found dictionary {DynamicPropertiesType.FullName}.{ValuesField.Name} ({dt.FullName})");
                    ...
                    {
                        Log.Out($"[WakaQuestProgression] Dictionary value type is {ValueEntryType.FullName}; probing for string member");
                        ...
                        if (ValueEntryStringField != null)
                            Log.Out($"[WakaQuestProgression] Entry string field: {ValueEntryStringField.Name}");
                        if (ValueEntryStringProperty != null)
                            Log.Out($"[WakaQuestProgression] Entry string property: {ValueEntryStringProperty.Name}");
                    }
                }
                ...
                {
                    Ready = true;
                    Log.Out("[WakaQuestProgression] Tier redirect armed");
                }
                else
                {
                    Log.Warning($"[WakaQuestProgression] Could not assemble read+write path on DynamicProperties (canRead={canRead}, canWrite={canWrite}); redirect disabled");
                }
            }
            catch (Exception e)
            {
                Log.Error($"[WakaQuestProgression] Initialize failed: {e}");
            }
        }
        ...
        public static string ReadQuestId(object actionInstance)
        {
            ...
            {
                Log.Warning($"[WakaQuestProgression] ReadQuestId failed: {e.Message}");
            }
            return null;
        }
        ...
        public static bool WriteQuestId(object actionInstance, string value)
        {
            ...
            {
                Log.Warning($"[WakaQuestProgression] WriteQuestId failed: {e.Message}");
            }
            return false;
        }
        ...
        static void DumpTypeMembers(Type t, string label)
        {
            try
            {
                ...
                sb.AppendLine($"[WakaQuestProgression] === Member dump ({label}) for {t.FullName} ===");
                const BindingFlags bf = BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.DeclaredOnly;
                for (var cur = t; cur != null && cur != typeof(object); cur = cur.BaseType)
                {
            ...
            {
                Log.Warning($"[WakaQuestProgression] DumpTypeMembers failed: {e.Message}");
            }
        }
        ...
        public static int GetPlayerPOITier(EntityPlayerLocal player)
        {
            ...
            try
            {
                ...
                if (dpd == null)
                {
                    Log.Out("[WakaQuestProgression] DynamicPrefabDecorator unavailable");
                    return -1;
                }
                ...
                if (instance == null)
                {
                    Log.Out($"[WakaQuestProgression] No POI at {pos}");
                    return -1;
                }

                int tier = instance.prefab.DifficultyTier;
                Log.Out($"[WakaQuestProgression] POI DifficultyTier={tier}");
                if (tier < 1) return -1;
                if (tier > 6) tier = 6;
                return tier;
            }
            ...
            {
                Log.Warning($"[WakaQuestProgression] GetPlayerPOITier error: {e.Message}");
                return -1;
            }
        }
    }
